...
func (flow *BaseFlow) Initiate() {
	...
	flow.storage.Store(gbconst.FlowKeyFlowID, gbrand.MakeKSUID())
	flow.storage.Store(gbconst.FlowKeyFlowTrails, []string{})
	flow.storage.Store(gbconst.FlowKeyFlowError, nil)
	flow.storage.Store(gbconst.FlowKeyFlowOutcome, nil)
	return
}

func (flow *BaseFlow) Inherit(fore Flow) {
	flow.storage = &sync.Map{}
	flow.storage.Store(gbconst.FlowKeyFlowID, fore.GetID())
	flow.storage.Store(gbconst.FlowKeyFlowTrails, append(fore.GetTrails(), gbrand.MakeBase62String(8)))
	flow.storage.Store(gbconst.FlowKeyFlowError, nil)
	flow.storage.Store(gbconst.FlowKeyFlowOutcome, nil)
	return
}
...
func (flow *BaseFlow) GetID() string {
	id, _ := flow.storage.Load(gbconst.FlowKeyFlowID)
	return id.(string)
}

func (flow *BaseFlow) GetTrails() []string {
	trails, _ := flow.storage.Load(gbconst.FlowKeyFlowTrails)
	return trails.([]string)
}

func (flow *BaseFlow) HasError() bool {
	err, _ := flow.storage.Load(gbconst.FlowKeyFlowError)
	return err != nil
}
...
func (flow *BaseFlow) SetError(err error) {
	flow.storage.Store(gbconst.FlowKeyFlowError, err)
	return
}

func (flow *BaseFlow) GetOutcome() any {
	outcome, _ := flow.storage.Load(gbconst.FlowKeyFlowOutcome)
	return outcome
}

func (flow *BaseFlow) SetOutcome(outcome any) {
	flow.storage.Store(gbconst.FlowKeyFlowOutcome, outcome)
	return
}
...
func (flow *BaseFlow) RequireRawMessage(key string) gbjson.RawMessage {
	value, ok := flow.Require(key).(gbjson.RawMessage)
	if !ok {
